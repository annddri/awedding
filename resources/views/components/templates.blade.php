<section id="katalog" x-data="{ currentFilter: 'all' }" class="py-20 lg:py-28 bg-slate-50 border-t border-slate-200/80">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        <!-- Header -->
        <div class="flex flex-col md:flex-row md:items-end justify-between space-y-4 md:space-y-0">
            <div class="space-y-3">
                <span class="text-xs uppercase font-bold tracking-widest text-amber-600 bg-amber-50 px-3 py-1 rounded-full border border-amber-200">
                    Pilihan Desain Manten
                </span>
            ...
            <div class="flex flex-wrap gap-2 bg-white p-1.5 rounded-2xl border border-slate-200 shadow-sm self-start">
                <button @click="currentFilter = 'all'" 
                        :class="currentFilter === 'all' ? 'bg-rose-600 text-white shadow-sm' : 'text-slate-600 hover:text-slate-900'"
                        class="px-4 py-2 rounded-xl text-xs sm:text-sm font-semibold transition-all">
                    Semua (6)
                </button>
                <button @click="currentFilter = 'elegant'" 
                        :class="currentFilter === 'elegant' ? 'bg-rose-600 text-white shadow-sm' : 'text-slate-600 hover:text-slate-900'"
                        class="px-4 py-2 rounded-xl text-xs sm:text-sm font-semibold transition-all">
                    Elegant Gold
                </button>
                <button @click="currentFilter = 'rustic'" 
                        :class="currentFilter === 'rustic' ? 'bg-rose-600 text-white shadow-sm' : 'text-slate-600 hover:text-slate-900'"
                        class="px-4 py-2 rounded-xl text-xs sm:text-sm font-semibold transition-all">
                    Rustic & Nature
                </button>
                <button @click="currentFilter = 'islamic'" 
                        :class="currentFilter === 'islamic' ? 'bg-rose-600 text-white shadow-sm' : 'text-slate-600 hover:text-slate-900'"
                        class="px-4 py-2 rounded-xl text-xs sm:text-sm font-semibold transition-all">
                    Islami
                </button>
            </div>
        ...
        <div class="mt-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <!-- Card 1: Royal Gold -->
            <div x-show="currentFilter === 'all' || currentFilter === 'elegant'" 
                 x-transition:enter="transition ease-out duration-300 transform"
                 x-transition:enter-start="opacity-0 scale-95"
                 x-transition:enter-end="opacity-100 scale-100"
                 class="bg-white rounded-3xl overflow-hidden border border-slate-200/80 shadow-sm hover:shadow-xl transition-all duration-300 group flex flex-col justify-between">
                <div>
                    <div class="relative aspect-[4/3] bg-stone-900 overflow-hidden">
                        ...
                        <div class="absolute bottom-4 left-4 right-4">
                            <span class="text-amber-300 text-xs font-serif italic">Seri Premium</span>
                ...
                <div class="p-6 pt-0">
                    <a href="#harga" class="w-full py-3 px-4 rounded-xl border border-rose-500 text-rose-600 font-bold text-sm flex items-center justify-center space-x-2 hover:bg-rose-600 hover:text-white transition-colors">
            ...
            <div x-show="currentFilter === 'all' || currentFilter === 'rustic'" 
                 x-transition:enter="transition ease-out duration-300 transform"
                 x-transition:enter-start="opacity-0 scale-95"
                 x-transition:enter-end="opacity-100 scale-100"
                 class="bg-white rounded-3xl overflow-hidden border border-slate-200/80 shadow-sm hover:shadow-xl transition-all duration-300 group flex flex-col justify-between">
                <div>
                    <div class="relative aspect-[4/3] bg-stone-900 overflow-hidden">
                        ...
                        <div class="absolute bottom-4 left-4 right-4">
                            <span class="text-emerald-300 text-xs font-serif italic">Seri Nature</span>
            ...
            <div x-show="currentFilter === 'all' || currentFilter === 'islamic'" 
                 x-transition:enter="transition ease-out duration-300 transform"
                 x-transition:enter-start="opacity-0 scale-95"
                 x-transition:enter-end="opacity-100 scale-100"
                 class="bg-white rounded-3xl overflow-hidden border border-slate-200/80 shadow-sm hover:shadow-xl transition-all duration-300 group flex flex-col justify-between">
                <div>
                    <div class="relative aspect-[4/3] bg-stone-900 overflow-hidden">
                        <img src="https://images.unsplash.com/photo-1544077960-604201fe74bc?auto=format&fit=crop&w=800&q=80" 
                             ...
                             class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                        <div class="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent"></div>
                        <span class="absolute top-4 left-4 bg-teal-500 text-white font-bold text-[10px] uppercase tracking-wider px-3 py-1 rounded-full shadow">
                            Favorit Manten
                        </span>
                        <div class="absolute bottom-4 left-4 right-4">
                            <span class="text-teal-200 text-xs font-serif italic">Seri Syar'i</span>
